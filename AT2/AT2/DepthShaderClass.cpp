...
bool DepthShaderClass::Initialise(ID3D11Device* device, HWND hwnd)
{
	bool result;

	result = InitialiseShader(device, hwnd, L"../AT2/DepthVertexShader.hlsl", L"../AT2/DepthPixelShader.hlsl");
	if (!result)
		return false;
...
bool DepthShaderClass::InitialiseShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "Depth Vertex Shader", "Missing Shader File", MB_OK);
		}
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "Depth Pixel Shader", "Missing Shader File", MB_OK);
		}
...
void DepthShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, "Error compiling shader.  Check shader-error.txt for message.", "Depth Shader", MB_OK);
