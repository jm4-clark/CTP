...
bool TextureShaderClass::Initialise(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitialiseShader(device, hwnd, L"../AT2/texture.vs", L"../AT2/texture.ps");
	if (!result)
	{
		return false;
	}
...
bool TextureShaderClass::InitialiseShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "Texture Vertex Shader", "Missing Shader File", MB_OK);
		}
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "Texture Pixel Shader", "Missing Shader File", MB_OK);
		}
...
void TextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, "Error compiling shader.  Check shader-error.txt for message.", "Texture Shader", MB_OK);
...
//			OutputShaderErrorMessage(errorMessage, hwnd, vsFilename);
//		}
//		//otherewise it simply could not find the shader file itself
//		else
//		{
//			MessageBox(hwnd, " texture vertex shader", "Missing Shader File", MB_OK);
//		}
...
//	//needs to match the VertexType structure in the ModelClass & in the shader
//	polygonLayout[0].SemanticName = "POSITION";
//	polygonLayout[0].SemanticIndex = 0;
//	polygonLayout[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
//	polygonLayout[0].InputSlot = 0;
...
//
//	//get cout of elements in layout
//	numElements = sizeof(polygonLayout)/ sizeof(polygonLayout[0]);
//
//	//create the vertex input layout
...
//		return false;
//
//	//release vertex and pixel shader buffer as they are no longer needed
//	vertexShaderBuffer->Release();
//	vertexShaderBuffer = 0;
//
//	pixelShaderBuffer->Release();
//	pixelShaderBuffer = 0;
//
//	//setup the description of the dynamic matrix constant buffer in the vertex shader
//	matrixBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
//	matrixBufferDesc.ByteWidth = sizeof(MatrixBufferType);
//	matrixBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
//	matrixBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
...
//
//	//transpore the matrics to prepare them for the shader
//	worldMatrix = XMMatrixTranspose(worldMatrix);
//	viewMatrix = XMMatrixTranspose(viewMatrix);
//	projectionMatrix = XMMatrixTranspose(projectionMatrix);
//
